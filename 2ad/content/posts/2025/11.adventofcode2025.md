...
MMIX's `DIV` rounds the quotient toward negative infinity and leaves the remainder in the special register `rR`. For a positive divisor that remainder is already the Euclidean one, in `[0, m)`, and `GET` copies it out.
