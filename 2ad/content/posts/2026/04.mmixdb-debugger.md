...
Pi has no closed form in arithmetic operations that assembly language provides. Pi is computed
from a series expansion at the accuracy desired or required. The shortest route to pi from integer
...
operations. For about four digits of pi we need roughly 100,000 iterations.
...
Cnt     GREG    Terms                   # terms remaining
One     GREG    Scale                   # Scale, in a register
...
That narrows it to the increment. `b 35` stops on it and `l` prints the surrounding lines with
the offending line:

```text
> 35            ADDU    Den,Den,1               # next denominator
```
